...
final class SettingMainViewController: UIViewController, StoryboardBased {
    ...
    override func motionEnded(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
        ...
        switch motion {
        case .motionShake:
            viewModel.action.onNext(.shakeDetected)
        default:
            break
        }
    }
    ...
    private func bindOutput() {
        ...
        viewModel.state.compactMap { $0.signedOut }
...
extension SettingMainViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .push(let desination):
            switch desination {
            case .internal:
                viewModel.action.onNext(.didSelectInternal)
            }
        case .logout:
            viewModel.action.onNext(.signOutTapped)
        }
    }
}
